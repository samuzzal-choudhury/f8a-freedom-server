...
def convert_resp_to_text(json_stack_report):
    response_to_client = "\nSummary of Application stack report generated at : "
    response_to_client += json_stack_report.get("finished_at")
    ...
    for result in json_stack_report['result']:
        stack_info = result['user_stack_info']
        response_to_client += "1) # of application dependencies : "
        dependency_count = str(stack_info.get('analyzed_dependencies_count', '0'))
        ...
        for dep in stack_info.get('analyzed_dependencies', []):
            dependency_cve_display = ""
            for security in dep.get('security', []):
                dependency_cve_display += "\t---- has CVE '{cve}' with 'CVSS score' " \
                                         "{cvss}\n".format(cve=str(security.get['CVE']),
                                                         cvss=str(security['CVSS']))
            licenses = ','.join(dep.get('licenses', []))
            dependency_license_display = "\t---- has  licenses '{lic}'".format(lic=licenses)

            response_to_client += "\n\t--'{name}' with version '{ver}':\n\t{cve}\n\t{lic}".format(
                name=dep['name'], ver=dep['version'], cve=dependency_cve_display,
                lic=dependency_license_display)

        response_to_client += "\n\t"

        license_conflict = stack_info.get('stack_license_conflict', False)
        if license_conflict == False:
            response_to_client += "\"\nNo application stack level license conflict has been found\"\n\n"
        ...
        if isinstance(result['recommendation'], dict):
            ...
            for companion in recommendation.get('companion', []):
                dependency_name = companion['name']
                version = companion['version']
                companion_dependency_license = str(','.join(companion['licenses']))
                response_to_client += "\t--'{name}' with version '{ver}' has license(s) '{lic}'".\
                    format(name=companion['name'], ver=companion['version'], lic=companion_dependency_license)
                if companion['reason'] is not None:
                    display_reason = companion['reason'].split('Do')[0]
                    response_to_client += "\tReason: {}\n\t".format(display_reason)
                response_to_client += "\tConfidence levels: {}%\n\t".\
                    format(str(companion['cooccurrence_probability']))
        else:
            response_to_client += "3) NO additional dependencies have been suggested for thie application\n\t"
        response_to_client += "\n\t"
        response_to_client += "4) NO usage outlier application depedencies have been found\n\t"

        response_to_client += "\n\t"
        response_to_client += "5) NO alternative  application depedencies have been suggested\n\t"
