...
def auth_required(view):
    """Check if the request header f8a-secret-key is set correctly."""

    def wrapper(*args, **kwargs):
        secret = getenv('F8A_CHE_AUTH_SECRET')
        secret_header = request.headers.get('f8a-secret-key')
        if secret_header is None:
            raise HTTPError(401, 'Authentication failed - missing required header')
